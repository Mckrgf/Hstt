            ...
            switch (((Message) packet).getType()) {
                case chat://表示收到单聊包，
                    ...
                    if (TextUtils.isEmpty(msgBody))
                        return;
                    Log.i("【PacketListener】", "processPacket: "+msgBody);
                    String fromUser=packet.getFrom();
                    String toUser=packet.getTo();
                    String msgType=((Message) packet).getType().toString();
                    if (null!=msgType && !"".equals(msgType) && "chat".equals(msgType)){
                        msgType="1";
                    }
                    // form,to消息类型,消息内容
                    SgeoEvent msg = new SgeoEvent();
                    msg.setEventId(SgeoEvent.IM_CHAT_REC);
                    msg.setFromId(fromUser);//发送者
                    msg.setToId(toUser);//接收者
                    msg.setMsgtype(msgType);//消息类型 1 chat
                    msg.setMsg(msgBody);//文本内容
                    //更新通知
...
//        BaseApplication.getMyAppContext().sendBroadcast(intent_nftc);
    }
